...
英検1級・2級・準2級・準1級の大問1（語彙）と、1級のトピック別表現を扱う静的Webアプリです。各級の過去問3回分に加え、1級の模試第1回〜第6回、準2級の自作模試第1回〜第4回、テーマ別5セット、国際医療福祉大学の総合型選抜基礎試験セットを収録しています（合計28セット）。準2級の自作模試と国際医療福祉大学セットは英検過去問を引用していません。
...
1級の通常問題（過去問3回分と模試第1回〜第6回、テーマ別問題は除外）では、ホームの「今日の学習」カードに新規問題の学習計画を表示できます。新規問題は、本番形式4択へ初めて回答した `(datasetId, q)` の組です。正誤は問わず、同じ問題の解き直し・意味だけ復習は日次・週次・総問題数へ重複加算しません。

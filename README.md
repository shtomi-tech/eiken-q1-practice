...
1. Discovery: 文脈から未知語の意味を推測する
2. Encoding: 同じ語の暗記カードで意味・補足情報を整理する
3. Retrieval: 文脈なしで意味チェックを行う
4. Application: 本番形式の4択問題を解く

文脈データがある語は `Context → 同じ語のFlash`、ない語は `Flash` から始めます。
文脈データは2級の6セットと、1級の全24セット（過去問3回分と模試第1回〜第21回、計2,364語句）に用意しています。
意味チェックはContextを表示せず、文脈なしのretrievalとして行います。

### 独立モード

- 意味だけ復習: Retrieval only。文脈を表示せず、日本語の意味4択だけを行います。
- 文脈推測練習: Inference only。ホームのContext Discoveryカードから開始します。
- 文脈→暗記カード試用: `contextLearn` として、文脈推測と暗記カードの関係だけを試せます。通常学習の進捗・FSRS・resumeとは共有しません。
- 最終チェック: Assessment only。全設問回答後に、文脈なしで総合確認を行います。
...
各級とも、その級の収録セットの語句をまとめた「意味だけ復習」をホーム画面の独立した「間隔復習」カードとして使えます。通常学習で最後まで解いた設問の語句だけが対象になり、未学習の語句は出題されません。1回の出題は最大30語句で、次にいつ出すかは語句ごとに **FSRS-6**（[ts-fsrs](https://github.com/open-spaced-repetition/ts-fsrs) を同梱）が決めます。固定の日数表ではなく、その語句の記録から「約9割思い出せるうちに出す」日を計算します。間隔の上限は180日です。

間隔の伸び方は**解答の速さ**で変わります。速い正解は「Good」、時間がかかった正解は「Hard」として扱い、Hardの語句は次回までの間隔が短くなります（遅いかどうかは、8秒未満なら常に速い・20秒以上なら常に遅い・その間はその回の中央値の1.6倍を境に判定します）。誤答した語句は「Again」として当日中（約10分後）にもう一度出題対象へ戻り、その語句の安定性が下がります。出題順は失敗回数・直近の解答の遅さ・復習期限の超過日数から決めます。解答直後には、その語句の前回までの平均秒数も表示します（出題から計測した回のみ）。ホームでは語句を「未実施」「要再確認」「3日以内」「1週間」「2週間」「1か月」「3か月」「半年以上」の8つに分けて表示します。対象語句は通常学習の進行に合わせて、その級の全語句まで増えます。
...
語源データは語根597個、A型832語、B型1,745語を収録しています。単語の語源表示は `data/word_origins.json` だけを参照します（語句データに語源テキストは持ちません）。英検1級模試第6回では、84語すべてにEtymonline・Merriam-Webster・Collins等を照合した語源チェーンを付け、参照URLを語ごとに記録しています。その他の1級セットも、語源説明またはC型の除外理由を各語に付けています。

2,255語のB型は、`word_origin_research.json`で原形ごとに再調査状況・出典・語源経路・語根候補を管理します。確認済みデータを追加するときは、バッチJSONを作成して次を実行します。
...
再調査対象2,255語はすべて確認済みです。`npm test`では台帳と表示用辞書の投影一致も検査します。

...
全級で、見出し語・発音記号・品詞・意味・例文・例文の日本語訳を同じ順序で表示します。熟語の暗記カードは、意味・核心イメージの連鎖・例文の3ブロックで表示し、不変化詞の共有イメージや仲間例のパネルは表示しません。単語の語源は収録されている場合に表示し、A型はチップと導出文、B型は由来の一行として表示します（語根そのものの解説パネルは表示しません）。

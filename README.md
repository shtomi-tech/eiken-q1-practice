...
1級の通常問題（過去問3回分と模試第1回〜第9回）では、ホームの「今日の学習」カードに新規問題の学習計画を表示できます。新規問題は、本番形式4択へ初めて回答した `(datasetId, q)` の組です。正誤は問わず、同じ問題の解き直し・意味だけ復習は日次・週次・総問題数へ重複加算しません。

- 総問題目標の初期値は、manifest配下の現在の1級通常問題数です。現在は291問・1164語句です。

...
意味だけ復習の間隔算出に使う FSRS-6 スケジューラ。CDN を参照せず実ファイルを同梱する（オフライン・可用性のため）。
...
`.github/workflows/pages.yml` の `Prepare static files` は**コピーするファイルを明示列挙している**。このディレクトリのファイルを増やす場合は workflow 側にも追記すること。漏れると本番だけ 404 になり、ローカルでは再現しない。`scripts/check-fsrs-vendor.cjs` がこの対応を検査している。

...
`apple-design` の原則は、見た目をApple風に寄せることではなく、押した瞬間の応答・現在位置からの連続性・操作中の可逆性・予測可能な空間関係を実装へ反映するために使う。固定操作バーとセッションの現在地バーは、紙面を隠しすぎない半透明のクロームとして扱い、Reduced Transparencyでは不透明な紙面へ戻す。
...
- 意味だけ復習の設問上部には音声操作だけを置き、定型的な「下線部の意味として最も適当なものを選べ」は表示しない
...
- ページレールは `max-width: 960px`・左右 `16px` padding とし、広い画面でのコンテンツレールは `928px` とする。
- `html { scrollbar-gutter: stable; }` で縦スクロールバーの有無による中央寄せレールの横移動を防ぐ。
- 問題セットの `.datasetUnitGrid` は2列・`gap: 8px`、`560px`以下では1列にする。設問一覧の `.itemList` は2列・`gap: 8px`、`720px`以下では1列にする。
- `320px`／`375px`幅でも横スクロールを発生させず、測定したコンテンツレールはそれぞれ `233px`／`288px` とする。長いラベルでカード高が変わることは許容し、必要な文言を固定高や省略で隠さない。
- `.sessionActionBar` は暗記カード専用の固定操作バーとし、ページのコンテンツレールと同じ幅・中央軸に揃える。`460px`以下でも前へ・カウンタ・次へを1行に収める。
- 短い見出し（`h1`・`h2`）は `text-wrap: balance` でまとまりよく改行し、本文は通常の折り返しとする。単語途中の `break-all` は使わない。
...
- **二次アクション（`.secondaryCta`）**: 透明地＋hairline枠、角丸md。reviewCta/finalCta系統は枠・文字をClayにし、意味復習の実行可能なCTA（`.meaningMissionCta.secondaryCta`）はSurface Strong地＋Clay 2px枠＋右矢印で、主CTAを増やさずに行動可能性を強調する
- **カード（`.card` 等）**: Card地・角丸lg・枠なし・影なし
- **語彙目標カード（`.vocabGoalCard`）**: 級単位の語彙目標を、前級目標→当級目標の1本のバーで示す。前級までの区間は「習得済み前提」の中立色（Muted）、このアプリでの実績だけをClayで重ねる。実績は差分区間でクランプし、0でないときは最小3pxで可視化する。現在位置には横向きドット絵のハリネズミ（`.vgHedgehog`／box-shadowのみ。外部画像を持たない、`aria-hidden`）が立ち、実績が1語句以上のときだけ歩く。英検5級〜1級ではカード上段に日次学習目標パネル（`.studyPlanPanel`）を統合し、下段にその級の語彙目標への到達予想・固定期間の理論語句予測・現在の収録語句数を追加する。5級は前級を持たない（起点0語）ため、中間の目盛りは出さない。医療福祉は語彙目標カード自体を出さない。語彙数は英検公式の公表値ではなく推定値だが、カード本体の前級注記は撤去済み（表示密度の判断）。推定値である旨は forecast 折りたたみ内の「理論上の学習量です／収録語句数」で担保する
- **学習計画パネル（`.studyPlanPanel`）**: 英検5級〜1級で、語彙目標カードの上段で、日次の正本を「今日 n / m問」として示す。通常問題の完了画面（`.doneBanner`）にも「今日の学習目標まであと n 問」または「✓ 今日の学習目標を達成しました」を表示する（意味だけ復習・最終チェックは対象外）。主CTAは増やさない。設定フォームは同パネル内で開閉し、1日の問題目標のnumber input、週開始曜日select、保存・キャンセルを持つ。総問題目標と週次集計の既存値・ロジックは互換性のため保持するが、設定・表示には出さない。進捗は文字と `role="progressbar"` の値を併記し、320px幅でも折り返す
- **間隔復習カード（`.spacedReviewCard`）**: 通常学習カードの兄弟として置き、Clayの左4px罫だけで復習導線を示す。カード内のCTAは意味復習セッションだけを開始する。意味復習で誤答した語句は、セッション末尾に暗記カードで見直し、「確認した」を押してから結果へ進む。完了画面では、今回の回答結果とは別に「今すぐ復習する残り：n語句」を表示する。間隔の内訳は固定の日数ラベルではなくバケット（未実施／要再確認／3日以内／1週間／2週間／1か月／3か月／半年以上の8つ）で示す。FSRSは連続値の間隔を返すため、「n日後」という約束をUIに書かない。ホーム上部に主CTA（`primary`）があるとき（通常学習の途中保存＝coreResume を含む）は、カード内CTAを二次操作 `.secondaryCta.meaningMissionCta` へ落とし、上部の `.startCta` と同じ視覚強度へ上げない。coreResume 時にカード内へ「先に再開を」等の重複案内は置かない
- **セッション操作バー（`.sessionActionBar`）**: 暗記カード（flashステージ）だけ、送り操作を画面下部へ固定する。Parchment地＋上辺hairline、影なし、`.wrap` と同じコンテンツレールで中央寄せ。左に前カード、中央に「カード n / 4」、右に次カードを1行で置く。一覧へ戻る操作（`.sessionHeadBack`）はタイトル行の右端へ置く。完了画面の結果操作は従来のカード末尾の`.actions`を使う
- **操作クローム（`.top` `.sessionHeadBack` `.sessionActionBar` `.answerActions`）**: 紙面の階層を保つ半透明背景＋`backdrop-filter`を使い、下のコンテンツとの境界は薄いエッジで示す。Reduced Transparencyでは`--parchment`の不透明面に戻す。固定操作バーは、本文を覆う範囲を最小限にし、safe-areaを確保する
- **学習ステップ（`.stageBar`）**: 3ステップは罫線で連結せず、8px gapの個別タイルで示す。現在のステップはInk、完了済みはOK Text＋Soft面とし、色以外にもラベルと`aria-current`で状態を伝える
- **セッション種別ごとの進捗表記**: ユニット学習は `.stageBar`（3STEP）＋`.roundInfo`「4語句の意味確認 n / 4」で位置を示す。「4語句」は1ユニット＝4語句のときだけの数なので、意味だけ復習（最大30語句）と最終チェック（全語句）では `.roundInfo` を出さず、`meaningBar`／`finalBar` に位置・正誤数を集約する。最終チェックでは重複する `.q1Progress` も表示しない。同じ「n / m」を3箇所以上に重複表示しない
- **意味だけ復習セッションの進捗表示**: セッション中はプール解放率のバー（旧「意味練習の解放状況」／級の全語句に対する解放数）を出さない。解放状況はホームの間隔復習カード（`.spacedReviewCard`）が正本。セッション中は `meaningBar` だけを置き、左ピル（active）に位置「n / m語句」、右ピルに「回答済 a / 正解 c」を出す。「残り n語句」の逆算表記は使わない
- **覚える（暗記カード）ステージの設問進捗（`.q1Progress`）**: このステージは全幅の塗りバー（`.q1ProgressTrack`／Clay塗り）を出さない。1問ずつ読み進める段階では設問位置は操作の判断に使えず、バーがカード見出しを画面下へ押し下げる。数値行「第n問 / m問・残りk問」とスクリーンリーダー用 `<progress>` は残す。塗りバーは確かめる（STEP 2）・解く（STEP 3）でのみ表示する
- **暗記カードの横スワイプ**: `.flash`上で10pxの判定閾値を越えた水平移動だけを受け付け、Pointer Captureで指に1対1追従させる。離すときは直近の速度をスプリングへ渡し、先頭／末尾ではrubber-bandで抵抗する。既存の前へ／次へボタンとキーボード操作は常に残す
- **動的セッション更新後のフォーカス**: カード・ステージを `renderSession()` でDOM置換した後は、描画完了地点で一度だけフォーカスを現在の内容へ移す。暗記カード（flashステージ）は現在語句 `.flashWord`、それ以外のステージはセッション見出し `#sessionStageTitle`（どちらも `tabindex="-1"`）。`focus({ preventScroll: true })` でスクロール位置は維持し、送り後の `scrollFlashCardIntoView` と競合させない。現在地は既存 `.flashNavCounter[aria-live="polite"]` が通知する。グローバルなキーボードショートカットは足さない
- **移動・再開ラベル**: セッションと完了画面から一覧へ戻る操作は「一覧へ戻る」、途中保存の再開操作は「続きから再開する」を基本とし、問題番号は括弧内に付記する
- **セッション中の保存表示（`.sessionState`）**: 保存先（この端末／クラウド）はヘッダーの `#shareStatus` に一本化する。生徒別利用では中断位置（`state.progress.resume`）もクラウド同期され別端末で再開できるため、セッション見出し下の保存案内文は表示しない
- **暗記カードの意味（`.flashMeaning`）**: 覚える対象として見出し語に次ぐ2番目の強さを持たせ、22px・600ウェイトを基準とする。見出し語に対する文字サイズ比は0.70以上を保つ
- **問題セットUnitカード（`.datasetUnitCard`）**: Parchment地＋hairline枠、角丸md。状態は未着手／途中保存あり／学習中／CLEARの4値で扱い、現在選択中はInkの左4px罫＋Surface Strong地（`aria-current="true"`）、CLEAR済みはOKの左4px罫＋「✓ CLEAR」文言、途中保存ありはMutedの「途中保存：第n問・…」文言を進捗と併記する。通常学習の誤答は専用の復習状態にせず、設問の回答結果として表示する。意味だけ復習の誤答見直しはセッション内だけで完結する。色だけに頼らず必ず文言を併記する。番号バッジは表示順であり永続IDではない。**現在選択中かつ通常学習の途中保存があるカード（`.isResumeStatus`）は操作要素にしない**：`onclick`・`type="button"`・操作矢印を持たず、`<div>` の状態表示コンテナとして描画する（操作ラベルは「途中保存あり」）。再開はホーム上部の主CTAへ集約する
- **現在Unitの開始・再開**: 保存なしの現在回は「この回を始める」とし、現在Unitカード自体がその開始操作を持つ。通常学習の途中保存がある現在Unitカードは操作要素にせず状態表示（上記 `.isResumeStatus`）にし、同じ保存位置を開く再開操作はホーム上部の `.startCta`「続きから再開する（第n問）」1つだけにする。別セットのUnitカードは従来どおり切替操作（`switchDataset`）を持ち、切り替え後にホーム上部の主CTAから再開する
...
- **単語の語源表示（`.originChain` `.originChip` `.originDerivation`）**: 単語の暗記カードでは、意味の直下・例文の前に置く。語源チェーン（`.originChain`）がある語は、熟語の核心イメージと同じ`.coreChain`のステップカードで、歴史的な構成要素から現在の中心義までを示す。チェーンの最終ステップは中心義だけにし、導出文は独立表示しない。A型はチップ（`.originChip`）と導出文（`.originDerivation`）を同じ語源ブロック内に置く。B型の未チェーン語は概要チップを表示せず、導出文がある場合だけ同じ語源ブロック内に表示する。単語カードでは情報量を抑えるため、語根そのものの解説パネル（語根名・イメージ注記・仲間語リスト）は表示しない。480px以下では共通のチェーンを縦積みにする
...
- **問題一覧フィルター（`.questionFilterBar`）**: 状態・種別の各グループに視覚ラベルと `role="group"` / `aria-label` を併記し、同名の「すべて」を区別する。状態フィルターは「すべて／未学習／途中／完了／不正解あり」。「不正解あり」は完了のサブ状態で、本番形式で誤答した設問（`unit.answerResult === "incorrect"`）だけを一致させる。誤答専用の復習キューは作らないが、この絞り込みで誤答した設問へ戻れるようにする
- **バッジ・チップ（`.tag` `.dictBadge` `.chip`）**: 角丸pill、hairline枠
- **フィードバックカード（`.feedback` `.resultBox`）**: 左4px太罫（Ink既定、OK/NGで色変化）＋Paper地、角丸md、外枠なし
- **文脈推測の回答フィードバック（`.contextChoicesAnswered` `.contextResult`）**: 回答後も4択を残して自分の回答と正解を同時に示す。正誤の視覚反応は明確にするがMeaning Check得点とは分け、結果カードには正誤タイトルと既存の訳だけを載せる。詳しい推測解説は加えず、結果確認後に暗記カードへ進む
- **完了画面の誤答導線（`.actions`）**: 誤答または意味確認の誤りがある場合、次の設問への主CTAに加えて同じ設問をセッション内で再学習する二次CTAを置く。誤答専用の永続復習キューは作らない
- **完了バナー（`.doneBanner`）の主役（ユニット学習）**: `.big`（40px）は「この設問の4語をどれだけ意味把握できたか」＝ `meaningCorrect / checkOrder.length`。本番形式1問の正誤は `.big` ではなく補助行「本番形式：✓ 正解／! 不正解」で示す。締めの一番大きい表示を、単発の4択の当たり外れではなく学習成果にする。意味だけ復習・最終チェックの `.big` は従来どおり各セッションのスコア
- **最終チェックの予告（`.finalCheckNote`）**: 問題一覧セクション（`.pathHead`）に、全設問を終える前でもこのセットのゴールを1行で示す。未解放時は「全n問を学習すると最終チェック（全m語・正答率80%でCLEAR）。あとk問」、解放後は「挑戦できます」、CLEAR済みは実績を表示。Clayの左3px罫だけで最終チェック文脈を示し、塗りCTAは増やさない（起動画面の主CTAは1つの原則を維持）
...
- ジェスチャーは固定durationではなく、表示中の`transform`と離す瞬間の速度から臨界減衰スプリングで着地させる。Reduced Motionでは慣性の退場を省き、状態遷移だけを即時に行う
...
- スワイプの直接追従は操作の因果性を保つため残すが、離した後のスプリング・回転・不透明度変化は行わず、次のカードまたは元位置へ即時に戻す
...
| C | どこまで来たか | 語彙目標カード（英検5級〜1級は日次学習目標を上段に統合）＋長期語彙予測 |
| D | 他に今日できることは | 意味だけ復習（範囲横断） |
| E | 別の範囲をやりたい | 学習セット／問題セットの選択 |
| F | 特定の箇所から始めたい | 範囲内の単位一覧・参照一覧 |
| G | 記録と設定を触りたい | 履歴・進捗リセット |

- 級の変更（URLで級を固定していないときのみ）は層Gに埋めず、先頭カード（「今日の学習」＝層B）の `.sectionHead` 右上へ `.gradeChangeButton`（`.ghost.smallGhost`）として置く。級を切り替えたい利用者が最初に見る位置で見つけられるようにするため。挙動（確認ダイアログ・`GRADE_KEY` 削除・`needsGradeChoice`）は変えない。

| 層 | kobun-vocab-learning | eiken-q1-practice |
|---|---|---|
| A | hero（初回のみ） | hero（初回のみ） |
| B | 主カード（label / h2 / recommend / stats） | 今日の学習カード（主CTA） |
| C | 語彙目標カード | 語彙目標カード（英検5級〜1級は日次目標を統合。級ごとの目標到達予想・期間別予測を含む） |
| D | 意味だけ復習 | 意味だけ復習 |
| E | 学習セット選択 | 問題セット選択 |
| F | 学習ブロックマップ → 単語一覧 | 問題一覧 |
| G | 履歴 → その他 | その他（進捗リセット）。級変更は先頭カード右上へ |

- EとFは隣接させ、「範囲を選ぶ → その中の単位を選ぶ」の順にする。
- DはEより上に置く。意味だけ復習は範囲横断のプールを対象にする。
- 進捗の数値（stats）と語彙目標カードの間に他要素を挟まない。
- 英検5級〜1級では日次学習目標を語彙目標カード内の上段へ置く。目標レコードは級ごとに独立して保存する。週開始曜日の設定と週次集計・再配分のロジックは保持するが、週次情報はホームに表示しない。語彙予測は同カード内へ置く。主CTA、ホーム層B/Cの順序、意味だけ復習の位置は変えない。
- 一覧はデータ順・manifest順を維持し、進捗で並べ替えない。

### 各層で常時表示する進捗は1つ

進捗表現の氾濫を避けるため、各層が常時見せる「進捗の数字」は1つに絞り、残りは同じカード内の折りたたみ（`<details>`。`.vocabForecast` と同じ操作パターン＝44px以上のsummary・`＋`/`−`の開閉記号・閉時は`summary`以外を隠す）へ入れる。折りたたみのsummaryには結論の1行を出し、閉じたままでも大枠が分かるようにする。

| 層 | 常時表示（1つ） | 同カード内の折りたたみ |
|---|---|---|
| B 学習目標（`.studyPlanPanel`） | 「今日 n / m問」（`.studyPlanMetrics` に1指標）。英検5級〜1級ではCの語彙目標カード内に置く | なし。週開始曜日は設定フォームで指定する |
| C 語彙目標（`.vocabGoalCard`） | 見出し＋「n 語 / m語」＋バー・目盛り・ハリネズミ・励ましメッセージ（すべて常時表示。ハリネズミの現在地が主モチベーションなので折りたたまない） | `.vocabForecast`：級ごとの目標到達予想・期間別予測だけを折りたたむ |
...
- 学習計画・語彙予測の設定フォームはカード内に温存する。総問題目標と週次集計・再配分の既存値・ロジックは保存形式と集計互換のため保持するが、ホームの設定・表示には出さない。
- 初回訪問・学習途中・全問CLEAR後で、カードの出し分け（表示/非表示）は変えない。折りたたみの初期状態も一律「閉」でよい。

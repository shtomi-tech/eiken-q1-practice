...
- **学習計画パネル（`.studyPlanPanel`）**: 英検5級〜1級で、語彙目標カードの上段で、日次の正本を「今日 n / m問」として示す。主CTAは増やさない。設定フォームは同パネル内で開閉し、1日の問題目標のnumber input、週開始曜日select、保存・キャンセルを持つ。総問題目標と週次集計の既存値・ロジックは互換性のため保持するが、設定・表示には出さない。進捗は文字と `role="progressbar"` の値を併記し、320px幅でも折り返す

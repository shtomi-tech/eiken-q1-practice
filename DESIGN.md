...
- **セッション種別ごとの進捗表記**: ユニット学習は `.stageBar`（3STEP）＋`.roundInfo`「4語句の意味確認 n / 4」で位置を示す。「4語句」は1ユニット＝4語句のときだけの数なので、意味だけ復習（最大30語句）と最終チェック（全語句）では `.roundInfo` を出さず、`meaningBar`／`finalBar` に位置・正誤数を集約する。同じ「n / m」を3箇所以上に重複表示しない

...
- **間隔復習カード（`.spacedReviewCard`）**: 通常学習カードの兄弟として置き、Clayの左4px罫だけで復習導線を示す。カード内のCTAは意味復習セッションだけを開始する。意味復習で誤答した語句は、セッション末尾に暗記カードで見直し、「確認した」を押してから結果へ進む。完了画面では、今回の回答結果とは別に「今すぐ復習する残り：n語句」を表示する。ホーム上部に主CTA（`primary`）があるとき（通常学習の途中保存＝coreResume を含む）は、カード内CTAを二次操作 `.secondaryCta.meaningMissionCta` へ落とし、上部の `.startCta` と同じ視覚強度へ上げない。coreResume 時にカード内へ「先に再開を」等の重複案内は置かない

...
- 設問として読ませる英文（意味だけ復習の `.askExample`）は「読む本文」であり、見出し用のセリフ体ではなく本文の Inter を使う。x-heightが大きく画面上の実効サイズと下線部（`--rule-underline`）の視認性を確保できる。サイズ20px・行間1.8・行幅上限58ch（`.flashEx` と同じ読み幅）。セリフ体は見出し（`h2`）と単語1語を掲げる `.askWord` に限る
...
- **語彙目標カード（`.vocabGoalCard`）**: 級単位の語彙目標を、前級目標→当級目標の1本のバーで示す。前級までの区間は「習得済み前提」の中立色（Muted）、このアプリでの実績だけをClayで重ねる。実績は差分区間でクランプし、0でないときは最小3pxで可視化する。現在位置には横向きドット絵のハリネズミ（`.vgHedgehog`／box-shadowのみ。外部画像を持たない、`aria-hidden`）が立ち、実績が1語句以上のときだけ歩く。1級ではカード上段に日次学習目標パネル（`.studyPlanPanel`）を統合し、下段に14,000語到達予想・固定期間の理論語句予測・現在の収録語句数を追加する。語彙数は英検公式の公表値ではなく推定値だが、カード本体の前級注記は撤去済み（表示密度の判断）。推定値である旨は forecast 折りたたみ内の「理論上の学習量です／収録語句数」で担保する
- **1級学習計画パネル（`.studyPlanPanel`）**: 語彙目標カードの上段で、日次の正本を「今日 n / m問」として示す。主CTAは増やさない。設定フォームは同パネル内で開閉し、1日の問題目標のnumber input、週開始曜日select、保存・キャンセルを持つ。総問題目標と週次集計の既存値・ロジックは互換性のため保持するが、設定・表示には出さない。進捗は文字と `role="progressbar"` の値を併記し、320px幅でも折り返す
...
- **セッション操作バー（`.sessionActionBar`）**: 暗記カード（flashステージ）だけ、送り操作を画面下部へ固定する。Parchment地＋上辺hairline、影なし、`.wrap` と同じコンテンツレールで中央寄せ。左に前カード、中央に「カード n / 4」、右に次カードを1行で置く。長いカードでも現在地と一覧への戻る操作を失わないよう、flashステージの戻る操作は上部のsticky補助バーに一本化する。完了画面の結果操作は従来のカード末尾の`.actions`を使う
- **操作クローム（`.top` `.sessionStickyNav` `.sessionActionBar` `.answerActions`）**: 紙面の階層を保つ半透明背景＋`backdrop-filter`を使い、下のコンテンツとの境界は薄いエッジで示す。Reduced Transparencyでは`--parchment`の不透明面に戻す。現在地バー・固定操作バーは、本文を覆う範囲を最小限にし、safe-areaを確保する
- **学習ステップ（`.stageBar`）**: 3ステップは罫線で連結せず、8px gapの個別タイルで示す。現在のステップはInk、完了済みはOK Text＋Soft面とし、色以外にもラベルと`aria-current`で状態を伝える
- **セッション種別ごとの進捗表記**: ユニット学習は `.stageBar`（3STEP）＋`.roundInfo`「4語句の意味確認 n / 4」で位置を示す。「4語句」は1ユニット＝4語句のときだけの数なので、意味だけ復習（最大30語句）と最終チェック（全語句）では `.roundInfo` を出さず、現在地バー（`.sessionStickyPos`）と `meaningBar`／`finalBar` に位置・正誤数を集約する。同じ「n / m」を3箇所以上に重複表示しない
...
- **セッション中の保存表示（`.sessionState`）**: 保存先（この端末／クラウド）はヘッダーの `#shareStatus` に一本化する。生徒別利用では中断位置（`state.progress.resume`）もクラウド同期され別端末で再開できるため、`.sessionState` に「この端末に保存」とは書かない。利用者にとっての意味だけを伝える固定文「中断してもこの位置から再開できます」を置く
...
| C | どこまで来たか | 語彙目標カード（1級は日次学習目標を上段に統合）＋長期語彙予測 |
...
| C | 語彙目標カード | 語彙目標カード（1級は日次目標を統合。14,000語到達予想・期間別予測を含む） |
| D | 意味だけ復習 | 意味だけ復習 |
| E | 学習セット選択 | 問題セット選択 |
...
- 1級では日次学習目標を語彙目標カード内の上段へ置く。週開始曜日の設定と週次集計・再配分のロジックは保持するが、週次情報はホームに表示しない。語彙予測は同カード内へ置く。主CTA、ホーム層B/Cの順序、意味だけ復習の位置は変えない。
...
|---|---|---|
| B 学習目標（`.studyPlanPanel`） | 「今日 n / m問」（`.studyPlanMetrics` に1指標）。1級ではCの語彙目標カード内に置く | なし。週開始曜日は設定フォームで指定する |
| C 語彙目標（`.vocabGoalCard`） | 見出し＋「n 語 / m語」＋バー・目盛り・ハリネズミ・励ましメッセージ（すべて常時表示。ハリネズミの現在地が主モチベーションなので折りたたまない） | `.vocabForecast`：1級の14,000語到達予想・期間別予測だけを折りたたむ |

"""英検2級 Chapter 3 模擬テスト第2回の内容と重複を検証する。"""
...
DATA_DIR = ROOT / "data"
VOCAB_PATH = DATA_DIR / "vocab_2_mock-2.json"
QUESTIONS_PATH = DATA_DIR / "questions_2_mock-2.json"
DATASET_ID = "eiken2-mock-2"
...
def check() -> None:
    ...
    if (len(questions), len(words), len(idioms)) != (20, 57, 23):
    ...
    if any(count not in range(3, 8) for count in answer_distribution):
        fail(f"正答位置の分布が3〜7件の範囲外です: {answer_distribution}")
    if sum(str(question.get("stem", "")).count("A:") for question in questions) != 9:
        fail("会話文の件数が原稿の9問と一致しません")
    ...
    print(f"{DATASET_ID}: content OK (20 questions / 80 items; 57 words, 23 idioms)")

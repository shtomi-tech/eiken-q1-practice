"""英検1級模試第9回の構造・内容・重複・原形表示・音声を検査する。"""
...
DATA_DIR = ROOT / "data"
DATASET_ID = "eiken1-mock-9"
QUESTIONS_PATH = DATA_DIR / "questions_1_mock-9.json"
...
AUDIO_DIR = ROOT / "assets" / "audio" / "vocab" / "1" / "mock-9"
...
def main() -> None:
    ...
    all_items = [*vocab.get("words", []), *vocab.get("idioms", [])]
    ...
    for q in range(1, 26):
        ...
        if any(not any(surface_variants(choice) & surface_variants(surface) for surface in surfaces) for choice in choices):
    ...
    flashcard_lemmas = {str(key).lower(): str(value).lower() for key, value in (lemmas.get("flashcardLemmas") or {}).items()}

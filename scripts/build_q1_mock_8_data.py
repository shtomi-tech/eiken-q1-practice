...
ETYMOLOGY = {
    "amiable": "ラテン語 amare（愛する）から、親しみやすく好意的な性質を表す。",
    "onerous": "ラテン語 onus（荷、負担）から、重い負担を伴う状態を表す。",
    "sporadic": "ギリシャ語 sporadikos（散らばった）から、散発的な状態を表す。",
    "incongruous": "ラテン語 incongruus（調和しない）から、場にそぐわないことを表す。",
    "grappled": "中英語 graplen（つかみ合う）に由来し、問題に取り組む意味へ広がった。",
    "dismantled": "古フランス語 desmanteler（覆いを外す）から、装置を解体する意味になった。",
    "scuffed": "英語 scuff（こすって傷をつける）に由来し、表面を擦る動作を表す。",
    "reviled": "ラテン語 re-（強く）＋vilis（卑しい）から、激しくののしる意味になった。",
    "diffused": "ラテン語 diffundere（広く注ぐ）から、広がり拡散する意味を表す。",
    "assailed": "古フランス語 assaillir（攻撃する）から、激しく襲う意味になった。",
    "maligned": "ラテン語 malignus（悪意のある）から、人を悪く言う意味を表す。",
    "shirked": "中英語 shirk（避ける、逃れる）に由来し、責任を怠る意味になった。",
    "furtively": "ラテン語 furtivus（盗みの、ひそかな）から、こっそり行う様子を表す。",
    "chronically": "ギリシャ語 chronikos（時間に関する）から、長く続く状態を表す。",
    "autonomously": "ギリシャ語 autos（自ら）＋nomos（規則）から、自律的なことを表す。",
    "equitably": "ラテン語 aequus（等しい、公平な）から、公平に扱う様子を表す。",
    "complexions": "ラテン語 complexio（組み合わせ、構成）から、顔色や肌の外観を表す。",
    "shimmers": "英語 shimmer は光が細かく揺れる様子を表す語として定着した。",
    "undertones": "under（下に）＋tone（色調、音調）から、表面下の色合いを表す。",
    "prophets": "ギリシャ語 prophetes（前もって語る人）から、預言者を表す。",
    "restitution": "ラテン語 restituere（元の状態へ戻す）から、返還や賠償を表す。",
    "vanity": "ラテン語 vanitas（空虚さ）から、うぬぼれや虚栄心を表す。",
    "enunciation": "ラテン語 enuntiare（明らかに述べる）から、明瞭な発音を表す。",
    "ambiguity": "ラテン語 ambiguus（両方向へ動く、曖昧な）から、二つ以上に解釈できることを表す。",
    "debonair": "フランス語 de bon air（よい雰囲気の）から、洗練され愛想のよい様子を表す。",
    "bucolic": "ギリシャ語 boukolikos（牛飼いの）から、田園的なものを表す。",
    "erroneous": "ラテン語 errare（さまよう、誤る）から、誤った状態を表す。",
    "pernicious": "ラテン語 pernicies（破壊、損害）から、有害で破壊的なことを表す。",
    "quagmire": "英語 quag（沼地）＋mire（ぬかるみ）から、泥沼や難局を表す。",
    "menace": "古フランス語 menacer（脅す）から、危害を及ぼす脅威を表す。",
    "mirage": "フランス語 mirage（幻影）から、遠くに見える幻想的な像を表す。",
    "plateau": "フランス語 plateau（平らな場所）から、高原や伸びが止まる状態を表す。",
    "persuade": "ラテン語 persuadere（十分に説得する）から、人を説得する意味になった。",
    "accentuate": "ラテン語 accentus（声の調子、アクセント）から、強調する意味を表す。",
    "inhibit": "ラテン語 inhibere（押さえつける）から、抑制する意味になった。",
    "absolve": "ラテン語 absolvere（解き放つ）から、責任や罪を免除する意味を表す。",
    "condolences": "ラテン語 condolere（共に苦しむ）から、遺族への弔意を表す。",
    "prologues": "ギリシャ語 prologos（前に語ること）から、物語の序章を表す。",
    "pretensions": "ラテン語 praetendere（前へ伸ばす、主張する）から、気取りや主張を表す。",
    "tribulations": "ラテン語 tribulare（苦しめる）から、試練や苦難を表す。",
    "deplete": "ラテン語 deplere（完全に満たす、空にする）から、資源を使い果たす意味になった。",
    "augment": "ラテン語 augere（増やす）から、量や力を増す意味を表す。",
    "smother": "中英語 smorther（煙で窒息させる）に由来し、火や感情を抑え込む意味にも広がった。",
    "repress": "ラテン語 reprimere（押し返す）から、感情や活動を抑圧する意味を表す。",
    "dexterity": "ラテン語 dexter（右側の、巧みな）から、手先の器用さを表す。",
    "contrition": "ラテン語 conterere（粉々に打ち砕く）から、心が砕けるような悔恨を表す。",
    "mayhem": "古フランス語 mahaime（身体的損傷）から、大混乱や暴力的な騒乱を表す。",
    "aridity": "ラテン語 aridus（乾いた）から、乾燥した状態を表す。",
    "bewail": "古英語 wail（嘆き声を上げる）から、嘆き悲しむ意味を表す。",
    "relinquish": "ラテン語 relinquere（後に残す、放棄する）から、手放す意味になった。",
    "transpose": "ラテン語 transponere（別の場所へ置く）から、位置や順序を入れ替える意味を表す。",
    "disparage": "古フランス語 desparagier（身分を下げる）から、価値を低く見る意味になった。",
    "opaque": "ラテン語 opacus（陰になった、暗い）から、光や意味を通さない状態を表す。",
    "avaricious": "ラテン語 avarus（貪欲な）から、財産を過度に欲しがる性質を表す。",
    "dank": "古ノルド語 dankr（湿った）に関係し、湿っぽく薄暗い状態を表す。",
    "venturesome": "ラテン語 ventura（起こること）からの venture（冒険）に由来し、冒険好きな性質を表す。",
    "consummate": "ラテン語 consummare（完全にする）から、完成された卓越した状態を表す。",
    "amateurish": "フランス語 amateur（愛好家）から、専門家でないような性質を表す。",
    "malignant": "ラテン語 malignus（悪意のある）から、悪性または有害なことを表す。",
    "neurotic": "ギリシャ語 neuron（神経）から、神経症に関する性質を表す。",
    "fallible": "ラテン語 fallere（だます、誤らせる）から、誤りやすいことを表す。",
    "tepid": "ラテン語 tepidus（ぬるい）から、温度や熱意が中途半端な状態を表す。",
    "murky": "中英語 mirk（暗闇）に関係し、濁ったり薄暗かったりする状態を表す。",
    "coarse": "古フランス語 cors（大きい、粗い）から、粗い質感や下品さを表す。",
    "ostracizing": "ギリシャ語 ostrakismos（陶片追放）から、集団から締め出す意味を表す。",
    "bridling": "古英語 bridel（馬勒、手綱）から、怒りを抑える意味にも広がった。",
    "lauding": "ラテン語 laudare（賞賛する）から、ほめたたえる意味を表す。",
    "pulverizing": "ラテン語 pulvis（粉、ほこり）から、粉末にする意味を表す。",
    "reverie": "フランス語 rêverie（夢想）から、ぼんやりとした空想を表す。",
    "gamut": "中世の音楽用語 gamma-ut から、音域全体や範囲を表す。",
    "tirade": "フランス語 tirer（引く）から、長く引き延ばした激しい演説を表す。",
    "discrimination": "ラテン語 discriminare（分けて区別する）から、識別や差別を表す。",
    "derogatory": "ラテン語 derogare（価値を減じる）から、軽蔑的なことを表す。",
    "bereft": "古英語 berēafian（奪う）から、何かを奪われた状態を表す。",
    "tactful": "ラテン語 tactus（触れること）から、相手に配慮して扱う巧みさを表す。",
    "laudatory": "ラテン語 laudare（賞賛する）から、賞賛を表す言葉になった。",
    "swagger": "英語 swagger（いばって歩く）に由来し、誇示する歩き方を表す。",
    "reconvene": "ラテン語 re-（再び）＋convenire（集まる）から、再招集する意味を表す。",
    "implode": "ラテン語 im-（内へ）＋plaudere（打つ）から、内側へ崩壊する意味を表す。",
    "seep": "古英語 seopian（ゆっくり染み出る）に由来し、液体や情報が徐々に広がる意味を表す。",
    "barrages": "フランス語 barrage（堰、遮断）から、集中砲撃や質問の集中を表す。",
    "blemishes": "中英語 blemisshen（傷つける）から、外見上のしみや欠点を表す。",
    "allegiances": "古フランス語 alegiance（忠誠の義務）から、忠誠や所属を表す。",
    "discrepancies": "ラテン語 discrepare（音が合わない）から、数字や事実の不一致を表す。",
    "sit in for": "sit（座る）＋in（中に入り）＋for（代わりに）から、代理を務める意味を表す。",
    "talk back to": "talk（話す）＋back（返して）＋to（相手へ）から、相手に口答えする意味を表す。",
    "walk out on": "walk（歩く）＋out（外へ）＋on（対象を残して）から、相手を見捨てて立ち去る意味を表す。",
    "pick up on": "pick（拾う）＋up（取り上げて）＋on（対象に）から、手がかりに気づく意味を表す。",
    "come around": "come（来る）＋around（周囲を回って）から、意識や考えが戻る意味を表す。",
    "spout off": "spout（噴き出す）＋off（外へ）から、言葉を次々に吐き出す意味を表す。",
    "roll back": "roll（転がす）＋back（後ろへ）から、状態や数値を元へ戻す意味を表す。",
    "bow out": "bow（身をかがめる）＋out（外へ）から、場から身を引く意味を表す。",
    "bring in": "bring（運ぶ）＋in（中へ）から、客や利益を中へもたらす意味を表す。",
    "mark out": "mark（印を付ける）＋out（外へ区切って）から、区域を示す意味を表す。",
    "trim down": "trim（整えて切る）＋down（下げて）から、量や大きさを削減する意味を表す。",
    "force down": "force（力を加える）＋down（下へ）から、無理に飲み込ませる意味を表す。",
    "got in on": "get in on（中へ入って対象に加わる）から、機会や計画に参加する意味を表す。",
    "capitalized on": "capitalize（資本化する）＋on（対象を利用して）から、機会を利益に結びつける意味を表す。",
    "wriggled out of": "wriggle（身をくねらせる）＋out of（外へ抜けて）から、困難や責任を巧みに逃れる意味を表す。",
    "shrank away from": "shrink（縮む）＋away（遠ざかって）＋from（対象から）から、恐れて避ける意味を表す。",
}

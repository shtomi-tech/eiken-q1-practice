"""公式1級Q1の語源説明から、原形キーの表示用語源辞書を同期する。"""
...
def sync() -> tuple[int, int]:
    ...
    added_origins = write_origin_entries(origins_path, new_origins)
    return added_origins, len(new_excluded)


def main() -> None:
    origins, excluded = sync()
    print(f"official origins: {origins}件追加 / C型除外 {excluded}件")

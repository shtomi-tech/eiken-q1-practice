...
function main() {
  ...
  for (const source of sources) {
    ...
    assert.equal(validateTargetSense({ ...item.source, ...item }, vocabulary.get(target)).status, "pass");
  ...
  const unchanged = (item) => item.q <= 5 || item.q >= 8;
  assert.deepEqual(runtime.contexts.filter(unchanged), baseline.contexts.filter(unchanged), "q=1..5 and q=8+ must remain unchanged");
